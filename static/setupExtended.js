...
export const setupConfigExtended = () => {
    ...
    const textmateConfigUrl = new URL('../syntaxes/ssdl.tmLanguage.json', window.location.href);
    extensionFilesOrContents.set('/language-configuration.json', languageConfigUrl);
    extensionFilesOrContents.set('/ssdl-grammar.json', textmateConfigUrl);
    ...
    return {
        wrapperConfig: {
            ...
            editorAppConfig: {
                $type: 'extended',
                languageId: 'ssdl',
                code: `// SSDL is running in the web!`,
                useDiffEditor: false,
                extensions: [{
                    config: {
                        name: 'ssdl-web',
